...
Waves::Waves(float xPos, float zPos, float xWidth, float zWidth, float meanHeight, Viewer *v) {
    ...
    for (int x = 0; x < N_MOBILES_X; x++) {
        for (int z = 0; z < N_MOBILES_Z; z++) {
            ...
            mobiles[idx].y = meanHeight + 0.05*sin(x*0.1) + 0.02*sin(x*0.5); 
     ...
     for (int x = 0; x < N_MOBILES_X; x++) {
        for (int z = 0; z < N_MOBILES_Z; z++) {
            ...
            if (mobiles[idx].isExciter && time < EXCITERS_TIME_LIMIT) {
                ...
                xm = (x == 0 ? meanHeight : mobiles[idx-N_MOBILES_Z].y);
                xp = (x == N_MOBILES_X-1 ? mobiles[idx%N_MOBILES_Z].y : mobiles[idx+N_MOBILES_Z].y);
                zm = (z == N_MOBILES_Z-1 ? mobiles[idx-N_MOBILES_Z+1].y : mobiles[idx+1].y);
                zp = (z == 0 ? meanHeight : mobiles[idx-1].y);
...
/*
                ...
                float dydx = (x == N_MOBILES_X-1 ? mobiles[idx%N_MOBILES_Z].y : mobiles[idx+N_MOBILES_Z].y);
                //float dydx = (x == N_MOBILES_X-1 ? meanHeight : mobiles[idx+N_MOBILES_Z].y);
                //dydx += (x == 0 ? meanHeight : mobiles[idx-N_MOBILES_Z].y);
                dydx += (x == 0 ? mobiles[idx+(N_MOBILES_X-1)*N_MOBILES_Z].y : mobiles[idx-N_MOBILES_Z].y);
                dydx -= 2 * mobiles[idx].y;
                float dydz = (z == N_MOBILES_Z-1 ? mobiles[idx-N_MOBILES_Z+1].y : mobiles[idx+1].y);
                //float dydz = (z == N_MOBILES_Z-1 ? meanHeight : mobiles[idx+1].y);
                //dydz += (z == 0 ? meanHeight : mobiles[idx-1].y);
                dydz += (z == 0 ? mobiles[idx+N_MOBILES_Z-1].y : mobiles[idx-1].y);
    ...
    for (int x = 0; x < N_MOBILES_X; x++) {
        for (int z = 0; z < N_MOBILES_Z; z++) {
            ...
            if (mobiles[idx].isExciter && time < EXCITERS_TIME_LIMIT) {
                ...
                float dvxdx = (x == N_MOBILES_X-1 ? mobiles[idx%N_MOBILES_Z].vx : mobiles[idx+N_MOBILES_Z].vx);
                //dvxdx += (x == 0 ? mobiles[idx].vx : mobiles[idx-N_MOBILES_Z].vx);
                dvxdx += (x == 0 ? mobiles[idx+(N_MOBILES_X-1)*N_MOBILES_Z].vx : mobiles[idx-N_MOBILES_Z].vx);
                dvxdx -= 2 * mobiles[idx].vx;
                //float dvzdz = (z == N_MOBILES_Z-1 ? 0.0f : mobiles[idx+1].vz);
                float dvzdz = (z == N_MOBILES_Z-1 ? mobiles[idx-N_MOBILES_Z+1].vz : mobiles[idx+1].vz);
                //dvzdz += (z == 0 ? 0.0f : mobiles[idx-1].vz);
                dvzdz += (z == 0 ? mobiles[idx+N_MOBILES_Z-1].vz : mobiles[idx-1].vz);
